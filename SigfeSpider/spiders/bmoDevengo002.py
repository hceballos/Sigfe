...
class SigfeSpiderSpider(scrapy.Spider):
    ...
    def obtener_orden_de_compra(self, response):
        try:
            return re.findall('(?<="af_inputText_content" type="text" value=").*?(?="><\/td><\/tr><\/tbody><\/table><table)', response.text)[1]
        except (IndexError):
            return re.findall('(?<="af_inputText_content" type="text" value=").*?(?="><\/td><\/tr><\/tbody><\/table><table)', response.text)[0]
    # =================================================================================================================================
    ...
    def modal(self, response):
        for x in re.findall('(?<=tabh::oc).*?(?=VisualizaVariacionPopup:nvPnDet::tabh::eoi)', response.text):
            print('linea : ', response.meta.get('linea') )
            print(" ")
            for i in (re.findall('(?<=<div id=").*?(?=" class="af_navigationPane-tabs_tab)',x) ):
                print(i)
                print( 'event', str(i))
                print( 'event.', str(i) ,'<m xmlns="http://oracle.com/richClient/comm"><k v="type"><s>action</s></k></m>')


    def pestana(self, response):
        print(response.text)
